...
{
    public class ManageEditor : BaseCommand
    {
        // + Parameter class ----------------------------------
        public enum Action
        {
        ...
        public class Args
        {
            [ToolParam("Operation to perform in editor", isRequired: true)]
            [ToolParamEnum(typeof(Action))]
            public Action? action;

            [ToolParam("Optional. If True, waits for certain actions.")]
        ...
        public static ToolMetaData ToolMetaData { get; set; } = 
            new ToolMetaData(
                name: "manage_editor",
                description: "Controls and queries the Unity editor's state and settings. Returns: Dictionary with operation results ('success', 'message', 'data').",
                paramType: typeof(Args),
                toolType: typeof(ManageEditor)
            );
        // ----------------------------------------------------
        ...
        public static async UniTask<Response> HandleCommand(Args args)
        {
            ...
            switch (args.action)
            {
                ...
                case Action.play:
                    try
                    {
                        if (!EditorApplication.isPlaying)
                        {
                            ...
                            var res = Response.Success("Entered play mode.");
                ...
                case Action.stop:
                    try
                    {
                        if (EditorApplication.isPlaying)
                        {
                            ...
                            var res = Response.Success("Exited play mode.");
                            res.isRecompileRequired = true;
                ...
                case Action.get_windows:
                    return GetEditorWindows();
                case Action.get_active_tool:
                    return GetActiveTool();
                case Action.get_selection:
                    return GetSelection();
                case Action.set_active_tool:
                    return SetActiveTool(args.toolName);
                // Tag Management
                case Action.add_tag:
                    return AddTag(args.tagName);
                case Action.remove_tag:
                    return RemoveTag(args.tagName);
                case Action.get_tags:
                    return GetTags(); // Helper to list current tags

                // Layer Management
                case Action.add_layer:
                    return AddLayer(args.layerName);
                case Action.remove_layer:
                    return RemoveLayer(args.layerName);
                ...
                default:
                    return Response.Error(
                        $"Unknown action: '{args.action}'. Supported actions include play, pause, stop, get_state, get_windows, get_active_tool, get_selection, set_active_tool, add_tag, remove_tag, get_tags, add_layer, remove_layer, get_layers."
                    );
            }
        }

        // --- Editor State/Info Methods ---
        private static Response GetEditorState()
        {
            try
            {
                var state = new
        ...
        private static Response GetEditorWindows()
        {
            try
            {
        ...
        private static Response GetActiveTool()
        {
            try
            {
        ...
        private static Response SetActiveTool(string toolName)
        {
        ...
        private static Response GetSelection()
        {
            try
            {
        ...
        private static Response AddTag(string tagName)
        {
        ...
        private static Response RemoveTag(string tagName)
        {
            if (string.IsNullOrEmpty(tagName))
        ...
        private static Response GetTags()
        {
            try
            {
        ...
        private static Response AddLayer(string layerName)
        {
        ...
        private static Response RemoveLayer(string layerName)
        {
            if (string.IsNullOrEmpty(layerName))
        ...
        private static Response GetLayers()
        {
            try
            {
