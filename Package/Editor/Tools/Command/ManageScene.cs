...
{
    public class ManageScene : BaseCommand
    {
        // + Parameter class ----------------------------------
        public enum Action
        {
        ...
        public class Args
        {
            [ToolParam("Operation to peform on editor.", isRequired: true)]
            [ToolParamEnum(typeof(Action))]
            public Action? action;

            [ToolParam("Scene name (no extension) for create/load/save.")]
            public string name;

            [ToolParam("Asset path for scene operations (default: \"Assets/\").")]
            public string path;

            [ToolParam("Build index for load/build settings actions.")]
        ...
        public static ToolMetaData ToolMetaData { get; set; } = 
            new ToolMetaData(
                name: "manage_scene",
                description: "Manages Unity scenes (load, save, create, get hierarchy, etc.). Returns: Dictionary with results ('success', 'message', 'data').",
                paramType: typeof(Args),
                toolType: typeof(ManageScene)
            );
        // ----------------------------------------------------
        ...
        public async static UniTask<Response> HandleCommand(Args args)
        {
            ...
            string relativeDir = args.path ?? string.Empty;
            ...
            if (string.IsNullOrEmpty(args.path) && args.action == Action.create) // Check original path for emptiness
            {
            ...
            string sceneFileName = string.IsNullOrEmpty(args.name) ? null : $"{args.name}.unity";
            ...
            if (args.action == Action.create && !string.IsNullOrEmpty(fullPathDir))
            {
                try
                {
            ...
            {
                case Action.create:
                    if (string.IsNullOrEmpty(args.name) || string.IsNullOrEmpty(relativePath))
                        return Response.Error(
                            "'name' and 'path' parameters are required for 'create' action."
                        );
                    return await CreateScene(fullPath, relativePath);
                case Action.load:
                    // Loading can be done by path/name or build index
                    if (!string.IsNullOrEmpty(relativePath))
                        return LoadScene(relativePath);
                    else if (args.buildIndex.HasValue)
                        return LoadScene(args.buildIndex.Value);
                    else
                        return Response.Error(
                ...
                case Action.save:
                    ...
                    return await SaveScene(fullPath, relativePath);
                case Action.get_hierarchy:
                    return GetSceneHierarchy();
                case Action.get_active:
                    return GetActiveSceneInfo();
                case Action.get_build_settings:
                    return GetBuildSettingsScenes();
                // Add cases for modifying build settings, additive loading, unloading etc.
                default:
                    return Response.Error(
                        $"Unknown action: '{args.action}'. Valid actions: create, load, save, get_hierarchy, get_active, get_build_settings."
                    );
            }
        }

        private async static UniTask<Response> CreateScene(string fullPath, string relativePath)
        {
        ...
        private static Response LoadScene(string relativePath)
        {
            if (
                !File.Exists(
                    Path.Combine(
        ...
        private static Response LoadScene(int buildIndex)
        {
        ...
        private async static UniTask<Response> SaveScene(string fullPath, string relativePath)
        {
            try
            {
        ...
        private static Response GetActiveSceneInfo()
        {
            try
            {
        ...
        private static Response GetBuildSettingsScenes()
        {
            try
            {
        ...
        private static Response GetSceneHierarchy()
        {
            try
            {
                Scene activeScene = EditorSceneManager.GetActiveScene();
